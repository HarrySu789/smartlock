...
一個基於 ESP32-S3 的多功能智慧門鎖系統，整合人臉辨識、指紋辨識、密碼輸入、OLED 顯示、電池備用電源、Telegram 遠端控制和超聲波近接喚醒功能。

## 📋 功能特色

- **超聲波近接喚醒** - HC-SR04 超聲波感測器，偵測 80cm 內有人時自動喚醒，無人時進入休眠模式，大幅省電
...
| 鍵盤 | 4×4 矩陣鍵盤 | 1 | |
| I2C 擴充板 | PCF8574 | 2 | 一個用於鍵盤，一個用於 LED + 超聲波 TRIG |
| 音頻擴大機 | MAX98357 | 1 | I2S 介面 |
| 繼電器模組 | 5V 繼電器 | 1 | 控制電磁鎖 |
| 超聲波測距 | HC-SR04 | 1 | 近接喚醒用 |
...
| HC-SR04 ECHO | D10 | GPIO9 | 經 1kΩ/2kΩ 分壓 |
...
├── README.md                # 本檔案
├── src/
│   ├── main.cpp             # 主程式（含狀態機、超聲波喚醒）
│   ├── config.h             # 所有設定（WiFi、密碼、腳位、超聲波）
│   ├── ultrasonic.h         # HC-SR04 超聲波感測器
...
```cpp
#define WAKE_DISTANCE_CM    80     // 距離 ≤ 80cm 時喚醒系統
...
// 近接喚醒設定（可選）
#define WAKE_DISTANCE_CM    80     // 喚醒距離（公分）
#define SLEEP_TIMEOUT_SEC   15     // 無人多久後進入休眠（秒）
```
...
1. **供電啟動**：系統上電後自動初始化所有硬體
2. **WiFi 連線**：連接到設定的 WiFi 網路
3. **進入休眠**：初始化完成後進入休眠模式等待喚醒
4. **超聲波偵測**：HC-SR04 每 200ms 量測一次距離
...
### Q: 超聲波量測值跳動很大
**A**: 
1. 確認 ECHO 分壓電阻焊接正確（量 D10 的電壓應在 3.0~3.4V 之間）
2. ECHO 線路不要太長（< 20cm），避免拾取雜訊
3. 使用 `measureMedianCm(5)` 取 5 次中位數（更穩定）
4. HC-SR04 量測間隔至少 30ms，不能太快

### Q: 休眠喚醒後人臉辨識很慢
**A**: 這是正常現象。鏡頭從 PSRAM 初始化需要幾十毫秒，第一幀可能較慢，第二幀起速度正常。
