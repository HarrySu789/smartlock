...
```
2S 18650 鋰電池 (7.4V~8.4V)
     │
     ├─── 10kΩ ──┬─── ESP32 D0 (GPIO1) ADC
     │            │
     │            ├─── 4.7kΩ ──── GND
     │            分壓比 = (10k+4.7k)/4.7k ≈ 3.127
     │            最大輸入 8.4V → 輸出約 2.68V（安全範圍）
     │
     └─── 100nF ──┬─── GND（濾波）
...
- **修復**：正確計算 rawADC 值，確保初始化區塊中的變數已定義
...
| 問題 | 解答 |
|------|------|
| 編譯失敗 "'raw' was not declared" | 請更新 src/battery.h 至最新版本 |
