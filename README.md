...
| **OLED 動態關閉** | 休眠時關閉顯示器，啟動時自動點亮 | 延長顯示器壽命 |
...
| 電阻 | 10kΩ 1/4W | 2 | CHRG 上拉 + **室內按鈕外部上拉** |
| 輕觸開關 | 4腳 Tactile Switch | 1 | 室內強制開門按鈕 |
...
└──────────────────────────┬──────────────────────────────────┘
    ...
    └────────────┘  └───────────┘  └──────────┘  └──────────┘
           ...
           ▼               ▼               ▼               ▼
    ┌────────────┐  ┌───────────┐  ┌──────────┐  ┌──────────┐
    │ pir_sensor.h │ │  keypad.h │  │ weather.h │  │ weather.h │
    │ （PIR）    │  │ （鍵盤）  │  │ （天氣）  │  │ （天氣）  │
    └────────────┘  └───────────┘  └──────────┘  └──────────┘
           │               │               │
           ▼               ▼               ▼
...
**PCF8574 #2（地址 0x21）— 純輸入板（室內按鈕 + PIR）**
...
> ⚠️ **重要**：本系統已移除 LED 與蜂鳴器功能，0x21 擴充板現在是「純輸入板」。
> 為了防止腳位浮接，**P2 與 P3 腳位必須接外部上拉電阻（10kΩ）到 3.3V**。
...
```
┌─────┬─────┬─────┬─────┐
│  1  │  2  │  3  │  A  │  ← A = 輸入管理員密碼後進入管理模式
├─────┼─────┼─────┼─────┤
│  4  │  5  │  6  │  B  │
├─────┼─────┼─────┼─────┤
│  7  │  8  │  9  │  C  │  ← C = 指紋驗證（7秒超時）
├─────┼─────┼─────┼─────┤
│  *  │  0  │  #  │  D  │  ← * = 清除輸入 / # = 確認密碼 / D = 刪除最後一位
└─────┴─────┴─────┴─────┘
```
...
|------|------|
| `0-9` | 將數字加入密碼輸入緩衝區 |
| `A` | 若已輸入正確管理員密碼，進入管理模式；否則顯示「Access Denied」 |
| `B` | 未使用 |
| `C` | 啟動指紋驗證模式（7秒超時）|
| `D` | 刪除輸入緩衝區的最後一個字元 |
| `*` | 清除輸入緩衝區 |
| `#` | 確認密碼：正確→解鎖開門，錯誤→失敗計數 |

#### 管理模式按鍵功能（需先輸入管理員密碼 + A）

| 按鍵 | 功能 |
|------|------|
| `1` | 新增人臉（透過 Telegram 指令 `/face_enroll`）|
| `2` | 新增指紋（需在指紋感測器上操作）|
| `3` | 列出已註冊人臉名單 |
| `4` | 刪除所有人臉（需再按 `#` 確認）|
| `*` | 返回待機狀態 |
...
### 🔥 邊緣觸發 (Edge-Triggered) 防連發機制

本系統採用「邊緣觸發」演算法來解決 PIR 感測器的連發問題：

```
傳統方式（電位觸發）：
  PIR HIGH → 觸發 → 維持 HIGH 期間持續觸發 ❌ 反覆觸發

本系統方式（邊緣觸發）：
  PIR: LOW → HIGH（上升緣） → 觸發一次 ✓
  PIR: HIGH → LOW（下降緣） → 不觸發
  
軟體實現：
  bool indoorPirEdgeTriggered = (currentIndoorPir && !lastIndoorPirState);
  lastIndoorPirState = currentIndoorPir;
```

**優勢**：
- 只有在人體「剛進入」偵測範圍的瞬間才觸發
- 解決天氣播報重複播放的問題
- 解決語音爆音問題

### 🛡️ I2C 擴充板保鑣機制

為防止 PCF8574 內部鎖存器因突波或接地而卡死，系統在每次 loop() 執行時強制寫入 HIGH：

```cpp
// 強制保鑣：每一圈都強制把 0x21 擴充板的 P2(按鈕) 與 PIR 設為 1 (輸入模式)
pcf8574_write(PCF_STATUS_ADDR, (1 << INDOOR_BTN_P) | (1 << PIR_IN_P));
```

### 🚪 室內一鍵開門功能

室內開門按鈕接在 PCF8574 #2 的 P2 腳位：
...
繼電器電源側（8.4V 動力迴路）：
  Com  ──────── 18650 電池正極 (LM2596S IN+)
  NO   ──────── 電磁鎖正極
  (電磁鎖負極直接接 GND)

邏輯說明：
  GPIO8 = HIGH → 繼電器不動作 → NC連通COM → 門鎖緊 ✓
  GPIO8 = LOW  → 繼電器動作   → NC斷開    → 門打開 ✓
  斷電時      → GPIO浮接      → 繼電器不動作 → 門鎖緊 ✓（安全）

⚡ 飛輪二極體防護（重要！）：
  在電磁鎖的正負極兩端並聯一顆 1N4007 二極體
  （銀色條紋端接正極），用以吸收斷電瞬間產生的反電動勢（突波）
  防止 ESP32 系統當機重啟
```
...
📌 並聯供電說明：
   LM2596S 的 5V 輸出端採「並聯」方式，直接分三路供給：
   1. ESP32 5V 腳位
   2. 繼電器 VCC
   3. MAX98357A 喇叭 VDD
   
   目的：避免大電流經過 ESP32 造成壓降，解決喇叭爆音問題

📌 濾波電容：
   • 在 LM2596S 的 5V 與 GND 輸出端之間
   • 並聯 220µF（推薦 1000µF）電解電容
   • 用以吸收系統高頻雜訊並提供瞬間大電流緩衝
   • 完美解決擴大機喇叭爆音問題
```
...
#### 1. src/relay.h - 繼電器邏輯修正
- **問題**：HIGH 觸發 + NO（常開）接線時，開鎖與鎖門動作相反
- **修復**：更新邏輯為：
  - `RELAY_PIN = HIGH` → 繼電器動作 → NO 閉合 → 門打開
  - `RELAY_PIN = LOW` → 繼電器不動作 → NO 打開 → 門鎖緊

#### 2. src/audio.h - WAV 播放無聲問題
- **問題**：天氣語音（WAV 檔案）無法正確播放
- **修復**：
  - 啟用 `FIX_UNSIGNED = true`（修正 WAV PCM 無符號格式）
  - 新增 Fallback 音效（當 WAV 播放失敗時自動切換到合成音效）

#### 3. src/main.cpp - 天氣播報除錯輸出
- **問題**：天氣語音沒有播放時缺乏診斷資訊
- **修復**：新增詳細除錯訊息，幫助排查問題
...
| 問題 | 解答 |
|------|------|
| 繼電器動作相反 | 請根據您的繼電器類型修改 src/relay.h 邏輯 |
| 天氣語音沒有聲音 | 查看序列監視器除錯輸出，嘗試將 FIX_UNSIGNED 改為 false 測試 |
...
- [Telegram BotFather](https://me/BotFather)
